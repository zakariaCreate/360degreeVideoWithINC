...
#include <bm/bm_sim/packet.h> // zakaria
#include <sys/stat.h> // zakaria
...
using namespace std;
namespace bm {
//sswitch_grpc::SimpleSwitchGrpcRunner myInstanceOfPacketSender ; //zakaria
//extern auto &runnerInstance = sswitch_grpc::SimpleSwitchGrpcRunner::get_instance() ;
//auto &runnerInstance = sswitch_grpc::SimpleSwitchGrpcRunner::get_instance(false, "0.0.0.0:9559", 1, "", 511) ;
//SimpleSwitchGrpcRunner runnerInstance = sswitch_grpc::SimpleSwitchGrpcRunner::SimpleSwitchGrpcRunner(1, NULL, 1, NULL, 1) ;
//sswitch_grpc::SimpleSwitchGrpcRunner runnerInstance ;
//SimpleSwitch* simpleSwitchInstance = runnerInstance.get_simple_switch_instance() ;
//TODO: CONTINUE HERE !!!!!!

struct ethernetheader {
  unsigned long int eth_dstAddr:48, eth_srcAddr:48 ; //48 bits
  unsigned short int etherType ; //2 bytes = 16 bits
};

struct ipheader {
  unsigned char      iph_ihl:4, iph_ver:4;  //1 byte = 8 bits = 4 bits
  unsigned char      iph_tos;  //1 byte = 8 bits
  unsigned short int iph_len;  //2 bytes = 16 bits
  unsigned short int iph_ident;  //2 bytes = 16 bits
  unsigned char      iph_flags:3;  //1 byte = 8 bits = 3 bits
  unsigned short int iph_offset:13;  //2 bytes = 16 bits = 13 bits
  unsigned char      iph_ttl;  //1 byte = 8 bits
  unsigned char      iph_protocol;  //1 byte = 8 bits
  unsigned short int iph_chksum;  //2 bytes = 16 bits
  unsigned int       iph_sourceip;  //4 bytes = 32 bits
  unsigned int       iph_destip; //4 bytes = 32 bits
};

struct udpheader {
  unsigned short int udph_srcport;  //2 bytes = 16 bits
  unsigned short int udph_destport;  //2 bytes = 16 bits
  unsigned short int udph_len;  //2 bytes = 16 bits
  unsigned short int udph_chksum; //2 bytes = 16 bits
	...
	class extern_example : public ExternType {
		public:
			...
			void method_example (const Data &ethDstAddr,
                           const Data &ethSrcAddr,
                           const Data &etherType,
                           const Data &version,
                           const Data &ihl,
                           const Data &diffserv,
                           const Data &totalLen,
                           ...
                           const Data &flags,
                           const Data &fragOffset,
                           const Data &ttl,
                           const Data &protocol,
                           const Data &hdrChecksum,
                           const Data &ipSrcAddr,
                           const Data &ipDstAddr,
                           const Data &udpSrcPort,
                           const Data &udpDstPort,
                           const Data &length_,
                           const Data &checksum,
                           const Data &egress_spec,
                           Data &packetSentSize){
				cout << "ID : " << get_packet().get_packet_id() << endl;
				cout << "get_payload_size : " << get_packet().get_payload_size() << endl;
				cout << "get_data_size : " << get_packet().get_data_size() << endl;
				cout << "get_ingress_length : " << get_packet().get_ingress_length() << endl;
				cout << "udpSrcPort int: " << udpSrcPort.get_int() << " END." << endl;
				cout << "ihl int: " << ihl.get_int() << " END." << endl;
				cout << "ip source int: " << ipSrcAddr.get_int() << " END." << endl ;
				cout << "ip destination int: " << ipDstAddr.get_int() << " END." << endl;
  			cout << "udpsize length_ int: " << length_.get_int() << " END." << endl;
				cout << "* identification int: " << identification.get_int() << " END." << endl;
				cout << "* totalLen int: " << totalLen.get_int() << " END." << endl;
				cout << "* flags int: " << flags.get_int() << " END." << endl;
				cout << "* fragOffset int: " << fragOffset.get_int() << " END." << endl;
				cout << "* Protocol int: " << protocol.get_int() << " END." << endl;

        //Constructing the ethernet frame
        char localPacketBuffer[1500];
        struct ethernetheader *ethTrans = (struct ethernetheader *) localPacketBuffer ;
        struct ipheader *ipTrans = (struct ipheader *) (localPacketBuffer + sizeof(struct ethernetheader)) ;
        struct udpheader *udpTrans = (struct udpheader *) (localPacketBuffer + sizeof(struct ipheader) + sizeof(struct ethernetheader)) ;


        ethTrans->eth_dstAddr = ethDstAddr.get_uint64() ; // 8 bytes = 64 bits = 48 bits
        ethTrans->eth_srcAddr =  ethSrcAddr.get_uint64() ; // 8 bytes = 64 bits = 48 bits
        ethTrans->etherType = etherType.get_uint() ; //2 bytes = 16 bits
        ipTrans->iph_ihl =  ihl.get_uint() ; //1 byte = 8 bits = 4 bits
        ipTrans->iph_ver =  version.get_uint() ;  //1 byte = 8 bits = 4 bits
        ipTrans->iph_tos = diffserv.get_uint() ;  //1 byte = 8 bits
        ipTrans->iph_len =  totalLen.get_uint() ;  //2 bytes = 16 bits
        ipTrans->iph_ident = identification.get_uint() ;  //2 bytes = 16 bits
        ipTrans->iph_flags =  flags.get_uint() ;  //1 byte = 8 bits = 3 bits
        ipTrans->iph_offset =  fragOffset.get_uint() ;  //2 bytes = 16 bits = 13 bits
        ipTrans->iph_ttl = ttl.get_uint() ;  //1 byte = 8 bits
        ipTrans->iph_protocol = protocol.get_uint() ;  //1 byte = 8 bits
        ipTrans->iph_chksum = hdrChecksum.get_uint() ;  //2 bytes = 16 bits
        ipTrans->iph_sourceip = ipSrcAddr.get_uint() ;  //4 bytes = 32 bits
        ipTrans->iph_destip = ipDstAddr.get_uint() ; //4 bytes = 32 bits
        udpTrans->udph_srcport = udpSrcPort.get_uint() ;  //2 bytes = 16 bits
        udpTrans->udph_destport = udpDstPort.get_uint() ;  //2 bytes = 16 bits
        udpTrans->udph_len = length_.get_uint() ;  //2 bytes = 16 bits
        udpTrans->udph_chksum =  checksum.get_uint() ; //2 bytes = 16 bits

        //show packet metadata header
        cout << "METADATA PACKET : " << endl ;
        cout << "eth_dstAddr " << ethTrans->eth_dstAddr << endl ;
        cout << "eth_srcAddr " << ethTrans->eth_srcAddr << endl ;
        cout << "etherType " << ethTrans->etherType << endl ;
        cout << "iph_ihl " << ipTrans->iph_ihl << endl ;
        cout << "iph_ver " << ipTrans->iph_ver << endl ;
        cout << "iph_tos " << ipTrans->iph_tos << endl ;
        cout << "iph_len " << ipTrans->iph_len << endl ;
        cout << "iph_ident " << ipTrans->iph_ident << endl ;
        cout << "iph_flags " << ipTrans->iph_flags << endl ;
        cout << "iph_offset " << ipTrans->iph_offset << endl ;
        cout << "iph_ttl " << ipTrans->iph_ttl << endl ;
        cout << "iph_protocol " << ipTrans->iph_protocol << endl ;
        cout << "iph_chksum " << ipTrans->iph_chksum << endl ;
        cout << "iph_sourceip " << ipTrans->iph_sourceip << endl ;
        cout << "iph_destip " << ipTrans->iph_destip << endl ;
        cout << "udph_srcport " << udpTrans->udph_srcport << endl ;
        cout << "udph_destport " << udpTrans->udph_destport << endl ;
        cout << "udph_len " << udpTrans->udph_len << endl ;
        cout << "udph_chksum " << udpTrans->udph_chksum << endl ;
        cout << endl ;

        for (int i=0; i!=42; i++){
            if (i >= 8) {
              localPacketBuffer[i-2] = localPacketBuffer[i] ;
            }
        }

        ofstream headerFile ;
        string headerFileName = "/home/p4/tutorials/exercises/tmpFiles/pictureFromPacket" ;
        headerFileName = headerFileName + to_string(ipSrcAddr.get_int()) + "_" + to_string(ipDstAddr.get_int()) + "_" + to_string(protocol.get_int()) + "_" + to_string(identification.get_int()) + ".header" ;


        //prepare udp source port
        string dstPortUdpString = to_string(udpDstPort.get_uint()) ;

				char bytes[4];
				int ip = ipSrcAddr.get_int() ;
        bytes[0] = ip & 0xFF ;
        bytes[1] = (ip >> 8) & 0xFF ;
        bytes[2] = (ip >> 16) & 0xFF ;
        bytes[3] = (ip >> 24) & 0xFF ;
        cout << "* ip source : " << (int)bytes[3] << "." << (int)bytes[2] << "." << (int)bytes[1] << "." << (int)bytes[0] << endl ;

        ip = ipDstAddr.get_int() ;
        bytes[0] = ip & 0xFF ;
        bytes[1] = (ip >> 8) & 0xFF ;
        bytes[2] = (ip >> 16) & 0xFF ;
        bytes[3] = (ip >> 24) & 0xFF ;
        cout << "* ip destination : " << (int)bytes[3] << "." << (int)bytes[2] << "." << (int)bytes[1] << "." << (int)bytes[0] << endl ;

				cout << "Start printing : " << endl ;

				//open file
				ofstream myfile ;
				string fileName = "/home/p4/tutorials/exercises/tmpFiles/pictureFromPacket" ;
				//fileName = fileName + to_string(get_packet().get_packet_id()) + ".jpg" ;
				fileName = fileName + to_string(ipSrcAddr.get_int()) + "_" + to_string(ipDstAddr.get_int()) + "_" + to_string(protocol.get_int()) + "_" + to_string(identification.get_int()) + ".jpg" ;
				int taille ;
				struct stat buf;
        if (stat(fileName.c_str(), &buf) != -1)
        {
          taille = totalLen.get_int()-20 ;
          cout << "Je ne le suis pas  !" << endl ;
        }else{
          cout << "Je suis le premier packet de la fragmentation !" << endl ;
          taille = totalLen.get_int()-8-20 ;
          headerFile.open(headerFileName, ios::out | ios::binary ) ;
          headerFile.write(dstPortUdpString.c_str()  , dstPortUdpString.length()) ; //save headers eth ip udp localPacketBuffer
          headerFile.close();
        }
				...
				while (i != taille){
					cout << get_packet().data()[i] ;
					//write in file
					myfile.write(get_packet().data()+i,1) ;
					i++ ;
				}
				//close file
				myfile.close() ;
        ...
        // what's below is useful for debuging
        //save packet metadata
        // ofstream metadataFile ;
				// string metadataFileName = "/home/p4/tutorials/exercises/tmpFiles/pictureFromPacket" ;
        // metadataFileName = metadataFileName + to_string(ipSrcAddr.get_int()) + "_" + to_string(ipDstAddr.get_int()) + "_" + to_string(protocol.get_int()) + "_" + to_string(identification.get_int()) + ".txt" ;
        // metadataFile.open(metadataFileName, ios::out | ios::binary | ios::app) ;
        // string metadata = to_string(ipSrcAddr.get_int()) + " " + to_string(ipDstAddr.get_int()) + " " + to_string(fragOffset.get_int()) +" "+to_string(flags.get_int())+'\n';
        // metadataFile.write(metadata.c_str(), metadata.length());
        // metadataFile.close();

				//check if it is the last fragment so we can transcode
				if (fragOffset.get_int() != 0 && (flags.get_int()&1) == 0 ) {
				  cout << "I am the last fragment" << endl ;
          ...
          //Constructing the ethernet frame
          char PacketBufferToSend[1514] ;
          char portUdpDestString[10] ;
          for (int i=0 ; i!=10 ; i++){
            //PacketBufferToSend[i] = '\0';
            portUdpDestString[i] = '\0';
          }

          struct ethernetheader *ethPcktCons = (struct ethernetheader *) PacketBufferToSend ;
          struct ipheader *ipPcktCons = (struct ipheader *) (PacketBufferToSend + sizeof(struct ethernetheader)) ;
          struct udpheader *udpPcktCons = (struct udpheader *) (PacketBufferToSend + sizeof(struct ipheader) + sizeof(struct ethernetheader)) ;
          char *payloadConstrPckt = PacketBufferToSend + sizeof(struct ipheader) + sizeof(struct ethernetheader) + sizeof(struct udpheader);

          ifstream headerFileForPacket ;
          headerFileForPacket.open(headerFileName, ios::in | ios::binary ) ;
          headerFileForPacket.read(portUdpDestString, 9) ;
          headerFileForPacket.close() ;


           unsigned long int addrLittleEndian, addrMacSrcBigEndian, addrMacDestBigEndian, ipSrcBigEndian, ipDestBigEndian ;
           unsigned long int b1, b2, b3, b4, b5, b6, b7, b8 ;
           addrLittleEndian = ethDstAddr.get_uint64() ;

           b1 = (addrLittleEndian & 0x00000000000000ff) << 40;
           b2 = (addrLittleEndian & 0x000000000000ff00) << 24;
           b3 = (addrLittleEndian & 0x0000000000ff0000) << 8;
           b4 = (addrLittleEndian & 0x00000000ff000000) >> 8;
           b5 = (addrLittleEndian & 0x000000ff00000000) >> 24;
           b6 = (addrLittleEndian & 0x0000ff0000000000) >> 40;

           addrMacDestBigEndian = b1 | b2 | b3 | b4 | b5 | b6  ;
           //
           addrLittleEndian = ethSrcAddr.get_uint64() ;
           ...
           addrMacSrcBigEndian = b1 | b2 | b3 | b4 | b5 | b6  ;
           //
           addrLittleEndian = ipSrcAddr.get_uint() ;

           b1 = (addrLittleEndian & 0x00000000000000ff) << 24;
           b2 = (addrLittleEndian & 0x000000000000ff00) << 8;
           b3 = (addrLittleEndian & 0x0000000000ff0000) >> 8;
           b4 = (addrLittleEndian & 0x00000000ff000000) >> 24;

           ipSrcBigEndian = b1 | b2 | b3 | b4   ;
           //
           addrLittleEndian = ipDstAddr.get_uint() ;
           ...
           ipDestBigEndian = b1 | b2 | b3 | b4   ;
           //
           ethPcktCons->eth_dstAddr = addrMacDestBigEndian ; // 8 bytes = 64 bits = 48 bits
           ethPcktCons->eth_srcAddr =  addrMacSrcBigEndian ; // 8 bytes = 64 bits = 48 bits
           ethPcktCons->etherType = ((etherType.get_uint() & 0x00ff)<<8) | ((etherType.get_uint()  & 0xff00)>>8); //2 bytes = 16 bits
           ipPcktCons->iph_ihl =  ihl.get_uint() ; //1 byte = 8 bits = 4 bits
           ipPcktCons->iph_ver =  version.get_uint() ;  //1 byte = 8 bits = 4 bits
           ipPcktCons->iph_tos = diffserv.get_uint() ;  //1 byte = 8 bits
           //ipPcktCons->iph_len =   ((1300 & 0x00ff)<<8) | ((1300  & 0xff00)>>8);  //2 bytes = 16 bits 1499
           ipPcktCons->iph_ident = ((get_packet().get_packet_id() & 0x00ff)<<8) | ((get_packet().get_packet_id()  & 0xff00)>>8) ;  //2 bytes = 16 bits
           ipPcktCons->iph_flags =  0 ;  //1 byte = 8 bits = 3 bits
           ipPcktCons->iph_offset =  0 ;  //2 bytes = 16 bits = 13 bits
           ipPcktCons->iph_ttl = ttl.get_uint() ;  //1 byte = 8 bits
           ipPcktCons->iph_protocol = protocol.get_uint() ;  //1 byte = 8 bits
           ipPcktCons->iph_chksum = 0 ;  //2 bytes = 16 bits
           ipPcktCons->iph_sourceip = ipSrcBigEndian ;  //4 bytes = 32 bits
           ipPcktCons->iph_destip = ipDestBigEndian ; //4 bytes = 32 bits
           udpPcktCons->udph_srcport =  ((9999 & 0x00ff)<<8) | ((9999  & 0xff00)>>8);  //2 bytes = 16 bits
           udpPcktCons->udph_destport =  ((atoi(portUdpDestString) & 0x00ff)<<8) | ((atoi(portUdpDestString)  & 0xff00)>>8);   //2 bytes = 16 bits
           //udpPcktCons->udph_len =  ((1280 & 0x00ff)<<8) | ((1280  & 0xff00)>>8);  //2 bytes = 16 bits 1479
           udpPcktCons->udph_chksum =  0 ; //2 bytes = 16 bits

          ifstream resizedImageFile ;
          resizedImageFile.open(fileName, ios::in | ios::binary ) ;
          unsigned int picturesize = resizedImageFile.readsome(payloadConstrPckt, 1458) ;
          resizedImageFile.close() ;
          unsigned short int taille_udp = picturesize+8, taille_ip = picturesize+28;
          udpPcktCons->udph_len = ((taille_udp & 0x00ff)<<8) | ((taille_udp  & 0xff00)>>8);  //2 bytes = 16 bits
          ipPcktCons->iph_len = ((taille_ip & 0x00ff)<<8) | ((taille_ip  & 0xff00)>>8);  //2 bytes = 16 bits

          //Calculation of ip checksum
          unsigned short *buf = (unsigned short *)ipPcktCons;
          int nwords = sizeof(struct ipheader) /2 /*+  sizeof(struct udpheader) */;
          unsigned long sum{0};
           ...
           unsigned short ipChecksum = (unsigned short)(~sum);
           //ipPcktCons->iph_chksum = ((ipChecksum & 0x00ff)<<8) | ((ipChecksum  & 0xff00)>>8);
           ipPcktCons->iph_chksum = ipChecksum;

           //Calculation of udp checksum
           sum = 0;
           sum += (ipPcktCons->iph_sourceip >>16)&0xFFFF;
           sum += (ipPcktCons->iph_sourceip )&0xFFFF;
           //the dest ip
           sum += (ipPcktCons->iph_destip >>16)&0xFFFF;
           sum += (ipPcktCons->iph_destip)&0xFFFF;
           //protocol and reserved: 17
           // unsigned short protoReservedTmp = 17 ;
           // sum += htons(protoReservedTmp);
           sum += htons(ipPcktCons->iph_protocol);
           //the length
           sum += udpPcktCons->udph_len;

           //nwords = sizeof(struct udpheader) ; taille_udp
           nwords = taille_udp ;
           unsigned short *buf1 = (unsigned short *)udpPcktCons;
           while (nwords > 1) {
                   sum += *buf1++;
                   nwords -= 2;
           }
           if(nwords > 0) {
              printf("+++++++++++++++padding: %dn", nwords);
              // sum += ((*buf1)&0xFF00);
              // protoReservedTmp = *buf1 ;
              // sum += htons(protoReservedTmp);
              sum += ((*buf1)&htons(0xFF00));
          }
           while (sum>>16) {
              sum = (sum & 0xffff) + (sum >> 16);
           }
           ...
           udpPcktCons->udph_chksum = ((unsigned short)sum == 0x0000)?0xFFFF:(unsigned short)sum;

           //show constructed packet header
           cout << "CONSTRUCTED PACKET : " << endl ;
           cout << "eth_dstAddr " << ethPcktCons->eth_dstAddr << endl ;
           cout << "eth_srcAddr " << ethPcktCons->eth_srcAddr << endl ;
           cout << "etherType " << ethPcktCons->etherType << endl ;
           cout << "iph_ihl " << ipPcktCons->iph_ihl << endl ;
           cout << "iph_ver " << ipPcktCons->iph_ver << endl ;
           cout << "iph_tos " << ipPcktCons->iph_tos << endl ;
           cout << "iph_len " << ipPcktCons->iph_len << endl ;
           cout << "iph_ident " << ipPcktCons->iph_ident << endl ;
           cout << "iph_flags " << ipPcktCons->iph_flags << endl ;
           cout << "iph_offset " << ipPcktCons->iph_offset << endl ;
           cout << "iph_ttl " << ipPcktCons->iph_ttl << endl ;
           cout << "iph_protocol " << ipPcktCons->iph_protocol << endl ;
           cout << "iph_chksum " << ipPcktCons->iph_chksum << endl ;
           cout << "iph_sourceip " << ipPcktCons->iph_sourceip << endl ;
           cout << "iph_destip " << ipPcktCons->iph_destip << endl ;
           cout << "udph_srcport " << udpPcktCons->udph_srcport << endl ;
           cout << "udph_destport " << udpPcktCons->udph_destport << endl ;
           cout << "udph_len " << udpPcktCons->udph_len << endl ;
           cout << "udph_chksum " << udpPcktCons->udph_chksum << endl ;
           cout << endl ;

           for (int i=0; i!=(taille_ip+16); i++){
               if (i >= 8) {
                 PacketBufferToSend[i-2] = PacketBufferToSend[i] ;
               }
           }
           // PacketBufferToSend[i-2]='\0';

           //print value of the buffer
           cout << "ZAKARIA : Print the value of the buffer constructed before decalage:" << endl ;
           for (int i=0 ; i!=42 ; i++){
             cout << i << " : " ;
             cout << bitset<8>( PacketBufferToSend[i] ) << endl ;
           }
           cout << endl ;
           ...
           auto &runnerInstance = sswitch_grpc::SimpleSwitchGrpcRunner::get_instance() ;
           SimpleSwitch* simpleSwitchInstance = runnerInstance.get_simple_switch_instance() ;
           simpleSwitchInstance->transmit_fn(egress_spec.get_int(), PacketBufferToSend, taille_ip+14);//1513
           unsigned long int packetSentSizeVar = taille_ip + 14 ;
           packetSentSize.set<int>(packetSentSizeVar);
           cout << "Packet transmitted through port  : " << egress_spec.get_int() << endl;
           cout << "Taille Packet " << taille_ip+14 << endl;
           cout << "Taille ip " << taille_ip << endl;
           cout << "Taille udp " << taille_udp << endl;
           cout << "picturesize " << picturesize << endl;
				}

        //show curent packet
        cout << "CURRENT PACKET : " << endl ;
        cout << "eth_dstAddr " << ethDstAddr.get_uint64()  << endl;
        cout << "eth_srcAddr " << ethSrcAddr.get_uint64()  << endl; // 8 bytes = 64 bits = 48 bits
        cout << "etherType " << etherType.get_uint()  << endl; //2 bytes = 16 bits
        cout << "iph_ihl " << ihl.get_uint()  << endl; //1 byte = 8 bits = 4 bits
        cout << "iph_ver " << version.get_uint()  << endl;  //1 byte = 8 bits = 4 bits
        cout << "iph_tos " << diffserv.get_uint()  << endl;  //1 byte = 8 bits
        cout << "iph_len " <<  totalLen.get_uint()  << endl ;  //2 bytes = 16 bits
        cout << "iph_ident " << identification.get_uint()  << endl;  //2 bytes = 16 bits
        cout << "iph_flags " <<  flags.get_uint()  << endl;  //1 byte = 8 bits = 3 bits
        cout << "iph_offset " <<  fragOffset.get_uint()  << endl;  //2 bytes = 16 bits = 13 bits
        cout << "iph_ttl " << ttl.get_uint()  << endl;  //1 byte = 8 bits
        cout << "iph_protocol " << protocol.get_uint()  << endl;  //1 byte = 8 bits
        cout << "iph_chksum " << hdrChecksum.get_uint()  << endl;  //2 bytes = 16 bits
        cout << "iph_sourceip " << ipSrcAddr.get_uint()  << endl;  //4 bytes = 32 bits
        cout << "iph_destip " << ipDstAddr.get_uint()  << endl; //4 bytes = 32 bits
        cout << "udph_srcport " << udpSrcPort.get_uint() << endl ;  //2 bytes = 16 bits
        cout << "udph_destport " << udpDstPort.get_uint()  << endl;  //2 bytes = 16 bits
        cout << "udph_len " << length_.get_uint()  << endl;  //2 bytes = 16 bits
        cout << "udph_chksum " <<  checksum.get_uint()  << endl; //2 bytes = 16 bits
        cout << endl ;
	...
	BM_REGISTER_EXTERN_METHOD(extern_example,method_example, Data &,  Data &,\
     const Data &, const Data &, const Data &, const Data &, const Data &, const Data &,\
      const Data &, const Data &, const Data &, const Data &, const Data &, const Data &, \
      const Data &, const Data &, const Data &, const Data &, const Data &, const Data &, Data &);

	BM_REGISTER_EXTERN_W_NAME(extern_ex, extern_example);
	BM_REGISTER_EXTERN_W_NAME_METHOD(extern_ex, extern_example, method_example, const Data &,\
     const Data &, const Data &, const Data &, const Data &, const Data &, const Data &, \
     const Data &, const Data &, const Data &, const Data &, const Data &, const Data &, \
     const Data &, const Data &, const Data &, const Data &, const Data &, const Data &, const Data &, Data &);
